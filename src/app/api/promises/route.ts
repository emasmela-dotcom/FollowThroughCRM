...
export async function POST(req: Request) {
  ...
  const { title, direction, person_id, due_at, notes, compensation } = body;
  if (!title?.trim()) return NextResponse.json({ error: "Title required" }, { status: 400 });
  const dir = direction === "they_owe" ? "they_owe" : "i_owe";
  const [row] = await sql`
    INSERT INTO promises (user_id, title, direction, person_id, due_at, notes, compensation)
    VALUES (${session.user.id}, ${String(title).trim()}, ${dir}, ${person_id || null}, ${due_at || null}, ${notes?.trim() ?? null}, ${compensation?.trim() ?? null})
