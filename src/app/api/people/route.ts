...
export async function POST(req: Request) {
  try {
    ...
    const rows = await sql`
      INSERT INTO people (user_id, name, email, phone, address, notes)
      VALUES (${session.user.id}, ${(name as string).trim()}, ${email?.trim() ?? null}, ${phone?.trim() ?? null}, ${address?.trim() ?? null}, ${notes?.trim() ?? null})
